...
class ResConfigSettings(models.TransientModel):
    ...
    app_bar_color = fields.Char(string='Appbar color',
                                config_parameter='theme_hue_backend.'
                                'appbar_color',
                                default='#000000')
    ...
    light_hover = fields.Char(string="Light Hover",
                              config_parameter='theme_hue_backend.light_hover',
                              default='#d5d5d5')
    appbar_text = fields.Char(string="Home Menu Text Color",
                              config_parameter='theme_hue_backend.appbar_text',
                              default='#F7F8F7')
    ...
    def config_color_settings(self):
        ...
        colors = {
            ...
            'appbar_color': self.env['ir.config_parameter'].sudo().get_param(
                'theme_hue_backend.appbar_color'),
            'primary_accent': self.env[
                'ir.config_parameter'].sudo().get_param(
                'theme_hue_backend.primary_accent_color'),
            'secondary_color': self.env[
                'ir.config_parameter'].sudo().get_param(
                'theme_hue_backend.secondary_color'),
            'kanban_bg_color': self.env[
                'ir.config_parameter'].sudo().get_param(
                'theme_hue_backend.kanban_bg_color'),
            'primary_hover': self.env[
                'ir.config_parameter'].sudo().get_param(
                'theme_hue_backend.primary_hover'),
            'light_hover': self.env[
                'ir.config_parameter'].sudo().get_param(
                'theme_hue_backend.light_hover'),
            'appbar_text': self.env[
                'ir.config_parameter'].sudo().get_param(
                'theme_hue_backend.appbar_text'),
            'appbar_hover': self.env[
                'ir.config_parameter'].sudo().get_param(
                'theme_hue_backend.appbar_hover')
        }
        return colors
